...
int readmultiplefocmec(char **focmecfiles, int nofiles, char *which_format, struct crust crst, double border, double dz, double dDCFS, struct tm reftime,
		double t0, double t1, double tfocmec, double mag, double ***focmec,	int **firstelements, int *NFM, int *NFM_timesel, struct eqkfm **eqkfm,int sel, int fm2){
// *firstelements should not be initialized (is done inside this function).
	...
	for (int n=0; n<nofiles; n++) {
		...
		if (cl>=0) ntotmax+=countline(focmecfiles[n]);
		else return 1;
	}
	if (fm2) ntotmax*=2;

	if (focmec) *focmec=dmatrix(1,4,1,ntotmax);
...
int readfocmec(char *focmecfile, char *which_format, struct crust crst, double border, double dz, double dDCFS, struct tm reftime,
...
// if focmec=NULL or eqkfm=NULL, they will be ignored.
    ...
    int NFMmax=(fm2==1)? 2*countline(focmecfile) : countline(focmecfile);
	double **focmec0;
	int *selected, *selectedsources, p;
	int NC= countcol(focmecfile);
